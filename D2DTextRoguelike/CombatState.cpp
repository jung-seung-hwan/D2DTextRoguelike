...
void CombatState::Update(PlayScene* pScene, float deltaTime)
{
    ...
    if (m_showEnemyDice)
    {
        m_enemyDiceTimer -= deltaTime;

        if (m_enemyDiceTimer <= 0.0f)
        {
            m_showEnemyDice = false;
            m_enemyDiceTimer = 0.0f;
        }
    }
...
void CombatState::Render(PlayScene* pScene, myspace::D2DRenderer* m_pRenderer, TextRenderer* pTextRenderer)
{
    ...
    if (m_showEnemyDice)
    {
        ...
        float offsetX = swing * 18.0f;
        float scale = 1.0f + fabsf(swing) * 0.04f;
        ...
        D2D1_RECT_F enemyDiceRect = D2D1::RectF(
            centerX - half,
            centerY - half,
            centerX + half,
            centerY + half
        ...
        {
            m_pRenderer->DrawBitmap(diceBitmap, enemyDiceRect);
        }

        wchar_t diceBuffer[32];
        swprintf_s(diceBuffer, 32, L"%d", m_enemyDiceNumber);
    ...
    nextBtn->SetOnClick([this, pScene]() // pScene 캡처
        {
            ...
            else if (m_combatManager.GetState() == BATTLESTATE::ENEMYTURN)
            {
                ...
                const DiceSystem::Result& enemyDice = m_combatManager.GetEnemyDice();

                if (!enemyDice.rolls.empty())
                {
                    m_enemyDiceNumber = enemyDice.rolls.back().roll;
                    m_showEnemyDice = true;
                    m_enemyDiceTimer = m_enemyDiceDisplayTime;
                }
